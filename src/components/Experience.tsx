...
// import { Briefcase, Users, Rocket, Award, Calendar , Globe } from 'lucide-react';

// const Experience = () => {
//   const experiences = [
//     {
//       title: 'Full Stack Web Development Intern',
//       organization: 'Dean International Relations Office @ PCCoE',
//       duration: '2025 June - Present',
//       type: 'Intern',
//       description: 'Building the official website for the International Grand Hackathon Challenge. Contributing to both frontend and backend systems, ensuring smooth user experience, responsiveness, and secure data flow.',
//       responsibilities: [
//         'Developing frontend and backend features for the hackathon website',
//         'Ensuring responsiveness and cross-browser compatibility',
//         'Implementing authentication, data handling, and API integrations',
//         'Deploying and maintaining the site using modern DevOps practices'
//       ],
//       icon: Globe,
...
const Experience = () => {
  ...
  return (
    <section id="experience" className="py-16 sm:py-20 bg-gray-900/30 px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12">
          <div>
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-6 sm:mb-8 flex items-center">
              <Briefcase className="w-5 sm:w-6 h-5 sm:h-6 mr-3 text-blue-400" />
              Experience & Leadership
            </h3>
            <div className="space-y-6">
              {experiences.map((exp, index) => (
                <Card key={index} className="bg-gray-800/50 border-gray-700 backdrop-blur-sm hover:bg-gray-800/70 transition-all duration-300">
                  <CardHeader className="pb-4">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
                      <div className="flex items-start sm:items-center space-x-3">
                        ...
                        <div>
                          <CardTitle className="text-base sm:text-lg text-white">{exp.title}</CardTitle>
                          <p className="text-blue-400 font-medium text-sm sm:text-base">{exp.organization}</p>
                        </div>
                      </div>
                  ...
                  <CardContent>
                    ...
                    <div className="space-y-2">
                      <h4 className="text-white font-medium mb-2 text-sm sm:text-base">Key Responsibilities:</h4>
                      <ul className="space-y-1">
                        {exp.responsibilities.map((resp, respIndex) => (
                          <li key={respIndex} className="text-gray-400 text-sm flex items-start">
                            <span className="text-blue-400 mr-2">•</span>
          ...
          <div>
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-6 sm:mb-8 flex items-center">
              <Award className="w-5 sm:w-6 h-5 sm:h-6 mr-3 text-purple-400" />
              Certifications & Courses
            </h3>
            <div className="space-y-6">
              {certifications.map((cert, index) => (
                <Card key={index} className="bg-gray-800/50 border-gray-700 backdrop-blur-sm hover:bg-gray-800/70 transition-all duration-300">
                  <CardContent className="p-5 sm:p-6">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                      <div>
                        <h4 className="text-base sm:text-lg font-semibold text-white mb-1">{cert.name}</h4>
                        <p className="text-blue-400 font-medium text-sm">{cert.issuer}</p>
                      </div>
                      <Badge
                        variant={cert.status === 'Completed' ? 'default' : 'secondary'}
                        className={cert.status === 'Completed'
            ...
            <Card className="bg-gradient-to-r from-blue-500/10 to-purple-500/10 border-blue-500/20 backdrop-blur-sm mt-6 sm:mt-8">
              <CardContent className="p-5 sm:p-6 text-center">
                <h4 className="text-lg sm:text-xl font-bold text-white mb-2">Non-Technical Skills</h4>
                <div className="flex flex-wrap justify-center gap-2 mt-3 sm:mt-4">
                  {['Leadership', 'Team Management', 'Event Coordination', 'Social Awareness', 'Public Speaking'].map((skill, index) => (
                    <Badge key={index} variant="secondary" className="bg-gray-700 text-gray-300 text-xs sm:text-sm">
                      {skill}
                    </Badge>
                  ))}
                </div>
